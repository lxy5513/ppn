...
def get_model():
    # load model
    model = coremltools.models.MLModel('./ppn.mlmodel')
    return model
...
def predict(model, image):
    ...
    input_name = model.get_spec().description.input[-1].name
    #  input_name = 'Input_0'
    output_name = model.get_spec().description.output[0].name
    #  output_name = 'sigmoid0'
    feature_map = model.predict({input_name: [image]})[output_name]
