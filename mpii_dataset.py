...
for keypoints in TRACK_ORDERS:
    ...
    ts = [KEYPOINT_NAMES.index(b) for a, b in pairwise(keypoints)] # no instance
    DIRECTED_GRAPHS.append([es, ts])


def get_mpii_dataset(insize, image_root, annotations,
                     train_size=0.8, min_num_keypoints=1, use_cache=False, seed=0):
    dataset_type = 'mpii'
    ...
    for anno in annotations:
        ...
        entry[1].append(np.array([x1, y1, x2 - x1, y2 - y1]))  # x, y, w, h 头部左下角坐标x, y
    ...
    test_set = KeypointDataset2D(
        ...
        bbox=[images[i][1] for i in test_images],#(left down (x, y) w h)
